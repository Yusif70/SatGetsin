...
namespace Advertisement.Service.Services.Concretes
{
    public class AuthService : IAuthService
    {
        ...
        public AuthService(RoleManager<IdentityRole> roleManager, UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, IConfiguration config, IMailService mailService, ISmsService smsService)
        {
        ...
        public async Task<ApiResponse> VerifyEmail(string userId, string token, int code)
        {
            if (verificationCodes.TryGetValue(userId, out var verification))
            {
                ...
                if (!res.Succeeded)
                {
                    return new ApiResponse { StatusCode = 500, Message = "Failed to confirm email." };
                }
        ...
        public async Task<ApiResponse> VerifyPhone(string userId, string token, int code)
        {
            if (verificationCodes.TryGetValue(userId, out var verification))
            {
                ...
                if (!res.Succeeded)
                {
                    return new ApiResponse { StatusCode = 500, Message = "Failed to confirm phone." };
                }
                await _signInManager.SignInAsync(user, isPersistent: true);
                return new ApiResponse { StatusCode = 200, Message = "Verification successful" };
            }
